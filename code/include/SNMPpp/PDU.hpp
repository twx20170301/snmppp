    ...
    class PDU
    {
        public:
            ...
            enum EType
            {
                ...
                kUnknown        = 0                 ,   // alias
                // SNMPv1 and higher:
                kGet            = SNMP_MSG_GET      ,   ///< SNMPv1+ get the specified OIDs
                kGetNext        = SNMP_MSG_GETNEXT  ,   ///< SNMPv1+ get the OID that comes after the one specified
                kResponse       = SNMP_MSG_RESPONSE ,   ///< SNMPv1+ ???
                kSet            = SNMP_MSG_SET      ,   ///< SNMPv1+ set the specified OID
                // SNMPv2 and higher:
                kGetBulk        = SNMP_MSG_GETBULK  ,   ///< SNMPv2+ get many OIDs at once
                kInform         = SNMP_MSG_INFORM   ,   ///< SNMPv2+ traps with replies
                kTrap2          = SNMP_MSG_TRAP2    ,   ///< SNMPv2+ traps
                kReport         = SNMP_MSG_REPORT       ///< SNMPv2+ ???
            };
             ...
             * clear() will leak memory if `snmp_free_pdu()` hasn't yet
             * been called.  @see free()
             */
